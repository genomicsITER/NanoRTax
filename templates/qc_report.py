...
if(os.path.exists(qc_report_path)):
    ...
    data_dict = {'Reads before QC': [data[0]], 'Reads after QC': [data[1]], 'Low quality reads': [data[2]], 'Filtered short reads': [data[3]],'Filtered long reads': [data[4]]}
    ...
    data_dict = {'Reads before QC': [new_report[0]], 'Reads after QC': [new_report[1]], 'Low quality reads': [new_report[2]], 'Filtered short reads': [new_report[3]],'Filtered long reads': [new_report[4]]}
    qc_df = pd.DataFrame.from_dict(data_dict)
    qc_df.to_csv("qc_report.csv", index=False)
