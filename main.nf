...
workflow.onComplete {
    ...
    def subject = "[nf-core/rtnanopipeline] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/rtnanopipeline] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/rtnanopipeline] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/rtnanopipeline] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/rtnanopipeline] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            // Catch failures and try with plaintext
            [ 'mail', '-s', subject, email_address ].execute() << email_txt
            log.info "[nf-core/rtnanopipeline] Sent summary e-mail to $email_address (mail)"
        }
    }
